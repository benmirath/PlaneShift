Shader "Custom/VertexManipulation/portal_bubble_2" {
	Properties {
	...
	SubShader {
		...
		// #pragma surface surf Lambert fullforwardshadows vertex:vert addshadow alpha
		// #pragma surface surf Lambert alpha
		#pragma surface surf Lambert fullforwardshadows addshadow alpha alphatest:alphaCutoff
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			// fixed2 _st = 1 + (st * -1);
			// st *= -1;
			// o.Albedo = tex2D(_MainTex, st) * st.y;
			
			// float distAdj = (1 - st.y) * 3.0;
			float distAdj = 1;
			distAdj *= smoothstep (st.y, 0.0, 0.4);
			
			o.Albedo = tex2D(_MainTex, (st + _Time.y * _AnimationSpeed)) + (tex2D(_MainTex, (st + _Time.y * _WaveHeight)) * (distAdj) * 10);
			// o.Albedo = fixed3 (1,0,0);
			// o.Metallic = 0;
			// o.Smoothness = 0;
			// o.Alpha = _Color.a * st.y;
			o.Alpha = _Color.a * distAdj;
			// o.Alpha =  1;
			// o.Alpha = 1;
			// o.Alpha = 1;
			// o.Alpha = 0;
