Shader "Custom/VertexManipulation/column_bubble_1" {
	Properties {
	...
	SubShader {
		...
		fixed4 ExpandVert (fixed4 _verts, fixed _spikeMag, fixed _animSpeed) {
			...
			fixed adjX = snoise ((_Time.y * _animSpeed) + _verts.yz) * _spikeMag;
			fixed adjZ = snoise ((_Time.y * _animSpeed) + _verts.xy) * _spikeMag; 
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			o.Smoothness = 0;
			// o.Alpha = _Color.a;
			// o.Emission = (o.Albedo * _Emission) * step (o.Albedo, _EmissionCutoff);
